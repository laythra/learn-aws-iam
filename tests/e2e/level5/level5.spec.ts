...
import { LEVEL_OBJECTIVES } from '@/levels/level5/objectives/level-objectives';
import { FIXED_POPOVER_MESSAGES } from '@/levels/level5/tutorial_messages/fixed-popover-messages';
import { POPOVER_TUTORIAL_MESSAGES } from '@/levels/level5/tutorial_messages/popover-tutorial-messages';
import { POPUP_TUTORIAL_MESSAGES } from '@/levels/level5/tutorial_messages/popup-tutorial-messages';
import {
  PolicyNodeID,
  ResourceNodeID,
  RoleNodeID,
...
const createServiceRole = async (
  ...
  await progress.expectLevelObjectiveCompleteToastAndClose(objectiveId);
};
...
test.describe('Stage 1 - IAM Roles Tutorial Introduction', () => {
  test('Initial Tutorial Flow', async ({
    tutorial,
    nodes,
    edges,
    entities,
    ...
    await test.step('Attach S3 role to policy and user', async () => {
      ...
      await attachS3RoleToPolicyAndUser(tutorial, nodes, progress);
    });
  });
...
test.describe('Stage 2 - Creating Service Roles in all orders', () => {
  test('Create Lambda first, then EC2', async ({
    nodes,
    entities,
    progress,
    tutorial,
  ...
  test('Create EC2 first, then Lambda', async ({
    nodes,
    entities,
    progress,
    tutorial,
